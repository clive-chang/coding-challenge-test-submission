...
export default async function handle(req: NextApiRequest, res: NextApiResponse) {
    ...
    const checkForNumeric = (value: string, msg: string) => {
        if (!isStrictlyNumeric(value)) {
            return res.status(400).send({
                status: 'error',
                errormessage: `${msg} must be all digits and non negative!`,
            });
        }
    };

    checkForNumeric(postcode as string, 'Postcode');

    checkForNumeric(streetnumber as string, 'Street Number')
